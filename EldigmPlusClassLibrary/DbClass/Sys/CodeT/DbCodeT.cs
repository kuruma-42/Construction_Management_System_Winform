...
namespace EldigmPlusClassLibrary.DbClass.Sys.CodeT
{
    public class DbCodeT
    {
        ...
        //SELECT 
        public DataSet sCodeTAuth(string TCODE, string SITE_CD )
        {
            string dbNm = sDbNm(SITE_CD);

            string sql = "" +
                " SELECT A.AUTH_CD, B.TCODE_NM, A.VIEW_FLAG, A.NEW_FLAG, A.MODIFY_FLAG " +
        ...
        public DataSet sCodeTAuthTtype(string TTYPE_SCD, string SITE_CD)
        {
            string dbNm = sDbNm(SITE_CD);
        ...
        //FIND TCODE WITH USING TCODE_NM
        public string SearchTcode(string TCODE_NM)
        {
            string sql = "" +

            " SELECT ISNULL(MAX(TCODE),'') TCODE FROM [PLUS-ELDIGM].dbo.T00_CODE_T " +
            " WHERE TCODE_NM = '" + TCODE_NM + "' ";
        ...
        //MODIFY AUTH 
        public int mCodeTAuth(string TCODE_NM, string SITE_CD, string AUTH_CD, string VIEW_FLAG, string NEW_FLAG, string MODIFY_FLAG)
        {
            string dbNm = sDbNm(SITE_CD);
            string TCODE = SearchTcode(TCODE_NM);
