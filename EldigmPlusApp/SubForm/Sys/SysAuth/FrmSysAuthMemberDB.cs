...
namespace EldigmPlusApp.SubForm.Sys.SysAuth
{
    public partial class FrmSysAuthMemberDB : Form
    {
        ...
        private void SetDataBind_CmbMember()
        {
            ...
            string reCode = "";
            string reMsg = "";
            Mem_WsSysAuthMemberDB.DataMemberMainDB[] getData = null;
            try
            {
            ...
            {
                logs.SaveLog("[error]  (page)::FrmSysAuthMemberDB.cs  (Function)::SetDataBind_CmbMember  (Detail):: " + "\r\n" + ex.ToString());
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
                    ...
                    if (dataGridView1.Rows[i].Cells["dgv1_AUTH_CD"].Value != null)
                    {
                        if (dataGridView1.Rows[i].Cells["dgv1_CHK"].Value != null)
                        {
                            if (dataGridView1.Rows[i].Cells["dgv1_CHK"].Value.ToString() == "1")
                            {
                                ...
                                reCode = wSvc.mSysAuthMemberDB(DBNM, sauthCd_val, sauthNm_val, myblockFlag_val, myconFlag_val, mycomFlag_val, myteamFlag_val, usingFlag_val, memo_val
                                    , out reMsg, out reData);
                                    ...
                                    if (Convert.ToInt16(reData) < 1)
                                    {
                                        ...
                                        if (usingFlag_val == "1")
                                        {
                                            reCode = wSvc.aSysAuthMemberDB(DBNM, sauthCd_val, sauthNm_val, myblockFlag_val, myconFlag_val, mycomFlag_val, myteamFlag_val
                                                , usingFlag_val, sauthLevel_val, memo_val, pInputId, out reMsg, out reData);
            ...
            {
                logs.SaveLog("[error]  (page)::FrmSysAuthMemberDB.cs  (Function)::btnSave_Click  (Detail):: " + "\r\n" + ex.ToString(), "Error");
            }
            finally
            {
                if (wSvc != null)
        ...
        private void SetDataBind_gridView2()
        {
            ...
            {
                logs.SaveLog("[error]  (page)::FrmSysAuthMemberDB.cs  (Function)::setDataBind_grideView2  (Detail):: " + "\r\n" + ex.ToString(), "Error");
            }
        }
