...
namespace EldigmPlusSvc_Memco.WebSvc.Sys.SysCode
{
    ...
    public interface IWsSysCodeGrp
    {
        ...
        [OperationContract]
        string mSysCode(string pScode, string pScodeNm, string pUsingFlag, string pSortNo, string pMemo, out string reMsg, out string reData);

        [OperationContract]
        string exSysCode(string pScode, out string reMsg, out string reData); //중복체크

        [OperationContract]
        string aSysCode(string pScode, string pScodeGrp, string pScodeNm, string pUsingFlag, string pMemo, string pSortNo, string pInputId, out string reMsg, out string reData); //추가
        
        [OperationContract]
        string fSysCodeSearch(string _scodeGrp, string pScode_Nm, out List<DataSysCode> reList, out string reMsg);
    }
